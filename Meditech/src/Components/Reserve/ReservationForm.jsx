...
const FormularioReserva = () => {
  ...
  const { colorMode: currentColorMode } = useColorMode();  // Acceder al modo de color actual
    ...
    return function (info) {
      ...
      if (dateStr === selectedDate) {
        return (
          <Box
          borderRadius="md"
          bg={currentColorMode === "dark" ? "blue.100" : "blue.500"}
          p={1}
          height="100%"
          width="100%"
        >
          <Box
            as="span"
            fontWeight="bold"
            color={currentColorMode === "dark" ? "black" : "white"}
          >
            {info.dayNumberText}
          </Box>
        </Box>
      ...
      return (
        <Box
          as="span"
          fontWeight="normal"
          color={currentColorMode === "dark" ? "white" : "black"}
        >
          {info.dayNumberText}
        </Box>
      );
  ...
  const statusTrue = () => {
    const result = disponibilties
      .filter((disponibility) => disponibility.status === true) // Filtrar solo aquellos con status true
      .map((disponibility) => ({
        title: "Disponible",
        id: disponibility.id,
  ...
  return (
    <Box marginTop="100px">
      ...
      eventContent={({ event }) => (
        <Box
          bg={currentColorMode === "dark" ? "purple.600" : "purple.300"}
          height="20px"
          borderRadius="8px"
          px={0.5}
          py={0.5}
      ...
      dayCellContent={renderDayCellContent(selectedDate)}
    />

              <Box>
                {fecha && (
                  <Text fontWeight="bold" mt={2}>
                    Usted está reservando para el:{" "}
                    {fecha &&
                      format(fechaLocal, "EEEE, dd 'de' MMMM 'de' y", {
                        locale: es,
                      })}
                  </Text>
                )}
                {fecha &&
                  hora
                    .filter((disponibility) => disponibility.date === fecha)
                    .sort((a, b) =>
                      a.timetable.startTime.localeCompare(b.timetable.startTime)
                    )
                    .map((disponibility, index) => {
                      return (
                        ...
                        >
                          ...
                          {disponibility.timetable.endTime}
                        </Button>
                      );
                    })}
              </Box>
              <Box>
                ...
                >
                  ...
                  personales relacionados con la salud y su transferencia al
                  especialista para concertar la cita. En Meditech, entendemos
                  que a veces surgen imprevistos y no puede asistir a su cita
                  programada. Sin embargo, para asegurarnos de que nuestros
                  médicos y el personal estén disponibles para todos nuestros
                  pacientes, requerimos que nos informe con al menos 24 horas de
                  antelación si no podrá asistir a su cita. En caso de que no se
                  presente a su cita sin previo aviso, se le cobrará una tarifa
                  por ausencia y no se le reembolsará el costo de la cita.
