    ...
    const result = await axios
      ...
      .then((res) =>
        res.data.map((specialist) => ({
          ...specialist,
          rating: specialist.reviews.reduce(
            (accumulator, currentValue, index, array) => {
              accumulator += currentValue.rating;
              if (index === array.length - 1) {
                return accumulator / array.length;
...
export const getName = (name) => {
  return async (dispatch) => {
    const result = await axios
      .get(`http://localhost:3001/doctors/name?name=${name}`)
      .then((res) =>
        res.data.map((specialist) => ({
          ...specialist,
          rating: specialist.reviews.reduce(
...
export const getAppointmentPatienById = (id) => {
  return async (dispatch) => {
    const result = await axios
      .get(`http://localhost:3001/patients/appointments/${id}`)
      .then((res) => res.data);
...
export const getPatientById = (id) => {
  return async (dispatch) => {
    const result = await axios
      .get(`http://localhost:3001/patients/dates/${id}`)
      .then((res) => res.data);
...
export const getAllPatientByName = (name) => {
  return async (dispatch) => {
    const result = await axios
      .get(`http://localhost:3001/patients/name?name=${name}`)
      .then((res) => res.data);
